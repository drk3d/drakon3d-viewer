...
<style>
*{box-sizing:border-box}body{margin:0;min-height:100vh;display:grid;place-items:center;background:#111;color:#fff;font:16px/1.45 system-ui,-apple-system,Segoe UI,sans-serif}.card{position:relative;width:min(100%,440px);padding:28px;border:1px solid #353535;border-radius:16px;background:#1c1c1c;box-shadow:0 16px 48px #0007}h1{padding-right:38px;font-size:24px;margin:0 0 16px}.preview{aspect-ratio:16/9;margin:0 0 22px;overflow:hidden;border:1px solid #353535;border-radius:10px;background:#111}.preview img{display:block;width:100%;height:100%;object-fit:cover}p{color:#c4c4c4;margin:0 0 22px}.close-dialog{position:absolute;top:14px;right:14px;width:30px;height:30px;padding:0;border:0;border-radius:7px;background:transparent;color:#b9b9bb;font:24px/1 system-ui,sans-serif;cursor:pointer}.close-dialog:hover{background:#303034;color:#fff}.actions{display:grid;gap:10px}.actions[hidden],.embed-panel[hidden],.embed-option[hidden]{display:none!important}.action{display:block;width:100%;padding:13px 16px;border:0;border-radius:9px;background:#fff;color:#111;text-align:center;font:inherit;font-weight:650;text-decoration:none;cursor:pointer}.secondary{background:#2c2c2c;color:#fff}.url{margin-top:18px;color:#aaa;font-size:12px;overflow-wrap:anywhere}.embed-panel{display:grid;gap:13px}.embed-panel h2{font-size:19px;margin:0}.embed-option{display:flex;justify-content:space-between;gap:16px;align-items:center;color:#ddd;font-size:14px}.embed-option input{width:17px;height:17px;accent-color:#fff}.embed-code{width:100%;min-height:150px;resize:vertical;border:1px solid #454545;border-radius:9px;background:#111;color:#eee;padding:10px;font:12px/1.45 ui-monospace,SFMono-Regular,Consolas,monospace}.embed-help{margin:0;color:#aaa;font-size:12px}</style>
</head><body><main class="card"><button class="close-dialog" id="close-share" type="button" aria-label="Close sharing window" title="Close">×</button><h1>Share model</h1><div class="preview"><img src="${escapedPreviewUrl}" alt="" onerror="this.parentElement.hidden=true"></div><div class="actions" id="share-actions"><button class="action" id="native-share" type="button">Share with an app</button><a class="action secondary" href="${safeHtmlTitle(whatsappUrl)}" target="_blank" rel="noopener">WhatsApp</a><a class="action secondary" href="${safeHtmlTitle(emailUrl)}">Email</a><a class="action secondary" href="${safeHtmlTitle(facebookUrl)}" target="_blank" rel="noopener">Facebook</a><button class="action secondary" id="embed-link" type="button">Embed</button><button class="action secondary" id="copy-link" type="button">Copy link</button></div><section class="embed-panel" id="embed-panel" hidden><h2>Embed model</h2><label class="embed-option">Hide header <input id="embed-hide-header" type="checkbox"></label><label class="embed-option" id="embed-hide-file-option">Hide File menu <input id="embed-hide-file" type="checkbox" checked></label><label class="embed-option">Fit 3D viewport to embed size <input id="embed-full-viewport" type="checkbox" checked></label><textarea id="embed-code" class="embed-code" readonly spellcheck="false" aria-label="Embed code"></textarea><p class="embed-help">This uses the same share link and preserves its password, expiry and permissions.</p><button class="action" id="copy-embed" type="button">Copy embed code</button><button class="action secondary" id="close-embed" type="button">Back</button></section><div class="url">${escapedShareUrl}</div></main><script>
...
document.querySelector('#close-share').addEventListener('click',()=>window.close());
