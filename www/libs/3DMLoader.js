...
function Rhino3dmWorker() {
	...
	function curveToPoints( curve, pointLimit ) {
		...
		// Determine and log curve type classification
		try {
			let typeName = "Unknown";
			let extraInfo = "";
			if ( curve instanceof rhino.LineCurve ) {
				typeName = "LineCurve";
			} else if ( curve instanceof rhino.PolylineCurve ) {
				typeName = "PolylineCurve";
				extraInfo = `points: ${curve.pointCount}`;
			} else if ( curve instanceof rhino.PolyCurve ) {
				typeName = "PolyCurve";
				extraInfo = `segments: ${curve.segmentCount}`;
			} else if ( curve instanceof rhino.ArcCurve ) {
				typeName = "ArcCurve";
				extraInfo = `angle: ${curve.angleDegrees.toFixed(1)}°`;
			} else if ( curve instanceof rhino.NurbsCurve ) {
				typeName = "NurbsCurve";
				extraInfo = `degree: ${curve.degree}, periodic: ${curve.isPeriodic}`;
			} else if ( curve && curve.constructor ) {
				typeName = curve.constructor.name || "Curve";
			}
			if ( curve && curve.domain ) {
				extraInfo += `, domain: [${curve.domain[0]}, ${curve.domain[1]}]`;
			}
			console.log(`[3DMLoader] curveToPoints classified curve as [${typeName}] - ${extraInfo}`);
		} catch (err) {
			console.warn('[3DMLoader] Failed to log curve type:', err);
		}
		...
		// ── Try to evaluate curve analytically if it represents an arc or circle ──
		try {
			let isArc = false;
			let arc = null;
			if ( typeof curve.tryGetArc === 'function' ) {
				const res = curve.tryGetArc();
				if ( res && res[ 0 ] ) {
					isArc = true;
					arc = res[ 1 ];
				}
			...
			if ( isArc && arc ) {
				...
				const startAngle = arc.startAngle ?? arc.StartAngle ?? 0;
				const endAngle = arc.endAngle ?? arc.EndAngle ?? 0;
				const plane = arc.plane;

				const cx = center.X ?? center.x ?? center[ 0 ] ?? 0;
				const cy = center.Y ?? center.y ?? center[ 1 ] ?? 0;
				const cz = center.Z ?? center.z ?? center[ 2 ] ?? 0;

				const xaxis = plane.XAxis ?? plane.xAxis;
				const yaxis = plane.YAxis ?? plane.yAxis;

				const ux = xaxis.X ?? xaxis.x ?? xaxis[ 0 ] ?? 1;
				const uy = xaxis.Y ?? xaxis.y ?? xaxis[ 1 ] ?? 0;
				const uz = xaxis.Z ?? xaxis.z ?? xaxis[ 2 ] ?? 0;

				const vx = yaxis.X ?? yaxis.x ?? yaxis[ 0 ] ?? 0;
				const vy = yaxis.Y ?? yaxis.y ?? yaxis[ 1 ] ?? 1;
				const vz = yaxis.Z ?? yaxis.z ?? yaxis[ 2 ] ?? 0;
				...
				for ( let j = 0; j < pointCount; j ++ ) {
					...
					const theta = startAngle + ( j / divisions ) * ( endAngle - startAngle );
				...
				if ( typeof plane.delete === 'function' ) plane.delete();
				if ( typeof arc.delete === 'function' ) arc.delete();

				console.log( `[3DMLoader] Successfully evaluated curve analytically as Arc with radius ${radius}` );
				return pts;
			}
		} catch ( e ) {
			console.warn( '[3DMLoader] tryGetArc check failed, falling back:', e );
		}

		// Convert PolyCurve to NurbsCurve for unified stable evaluation
		if ( curve instanceof rhino.PolyCurve ) {

			const nurbs = curve.toNurbsCurve();
			if ( nurbs ) {

				const res = curveToPoints( nurbs, pointLimit );
				nurbs.delete();
				return res;

			}

			// Fallback segment evaluation for PolyCurve if toNurbsCurve fails
			if ( curve instanceof rhino.PolyCurve ) {
				...
				for ( let i = 0; i < segmentCount; i ++ ) {
					...
					const segmentArray = curveToPoints( segment, pointLimit );
		...
		if ( curve instanceof rhino.NurbsCurve ) {

			const nurbs = curve.duplicate();

			if ( nurbs.degree === 1 ) {

				const pLine = nurbs.tryGetPolyline();
				if ( pLine ) {
			...
			for ( let j = 0; j < pointCount; j ++ ) {
				...
				rc.push( nurbs.pointAt( j === 0 ? domain[ 0 ] : ( j === pointCount - 1 ? domain[ 1 ] : t ) ) );
		...
		for ( let j = 0; j < pointCount; j ++ ) {
			...
			const t = domain[ 0 ] + ( j / divisions ) * ( domain[ 1 ] - domain[ 0 ] );
			rc.push( curve.pointAt( j === 0 ? domain[ 0 ] : ( j === pointCount - 1 ? domain[ 1 ] : t ) ) );
